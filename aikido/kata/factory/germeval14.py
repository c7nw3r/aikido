...
from aikido.kata.common.arrow_kata import DatasetKata


class GermEval14Metric(Metric):
    def __init__(self):
    ...
    def __call__(self, request: MetricRequest) -> dict:
        ...
        preds = [
            [GermEval14.labels()[p] for (p, l) in zip(prediction, label) if l != -100]
            for prediction, label in zip(np.argmax(request.preds, axis=2), request.refs)
        ]
        refs = [
            [GermEval14.labels()[l] for (p, l) in zip(prediction, label) if l != -100]
            for prediction, label in zip(np.argmax(request.preds, axis=2), request.refs)
        ]
...
class GermEval14:
    ...
    def __call__(self):
        ...
        encoded = dataset.map(function=self.tokenize_and_align_labels,
                              ...
                              remove_columns=['id', 'ner_tags', 'nested_ner_tags', 'source', 'tokens'])
